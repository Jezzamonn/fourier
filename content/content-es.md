Las transformadas de Fourier son una herramienta utilizada en un montón de cosas diferentes. Esta es una explicación de lo que hace una transformada de Fourier, algunas formas diferentes en que puede ser útil y cómo puedes hacer cosas bonitas con ella, como esta cosa:
...
Vamos a dejar las matemáticas y las ecuaciones fuera de eso por ahora. Hay un montón de matemáticas interesantes detrás de todo esto, pero es mejor comenzar con lo que realmente hace, y por qué querrías usarlo primero. Si deseas saber más sobre el cómo, ¡hay más sugerencias de lectura al final!
...
Necesitamos muchos de ellos esta vez, técnicamente una cantidad infinita para representarlos perfectamente. A medida que sumamos más y más ondas sinusoidales, el patrón se acerca cada vez más a la onda cuadrada con la que comenzamos.
...
*Arrastre el control deslizante de arriba para jugar con la cantidad de ondas sinusoidales que hay.*
...
Una vez más, aparte de la extravagancia extra, la ola se ve bastante similar con solo la mitad de las ondas sinusoidales.
...
Entonces, en este caso, podemos usar las transformadas de Fourier para comprender las propiedades fundamentales de una onda, y luego podemos usar eso para cosas como la compresión.
...
Ahora, al principio, dije que la transformada de Fourier divide las cosas en ondas sinusoidales. La cosa es que las ondas sinusoidales que crea no son solo ondas sinusoidales regulares, sino que son 3D. Podrías llamarlos "sinusoides complejos". O simplemente "espirales".
        
<canvas id="complex-sinusoid" class="sketch" width=500 height=500></canvas>
        
Si echamos un vistazo desde el lateral, se ven como ondas sinusoidales. Desde el frente, sin embargo, estos parecen círculos.
...
Como antes, obtenemos una buena aproximación de nuestro patrón con solo unos pocos círculos. Debido a que esta es una forma bastante simple, todo lo que hacen los últimos es hacer los bordes un poco más afilados.
...
*Utiliza el control deslizante para controlar cuántos círculos se utilizan para su dibujo.*
                
Una vez más, verás que para la mayoría de las formas, podemos aproximarlas bastante bien con solo un pequeño número de círculos, en lugar de guardar todos los puntos.
                
¿Podemos usar esto para datos reales? Bueno, podríamos! En realidad, tenemos otro formato de datos llamado SVG, que probablemente hace un mejor trabajo para los tipos de formas que tendemos a crear. Así que por el momento, esto es solo para hacer pequeños gifs geniales.
...
Para hacer eso, cada una de nuestras ondas sinusoidales también serán imágenes. En lugar de una ola que es una línea, ahora tenemos imágenes con secciones en blanco y negro. Para representar el tamaño de una onda, cada imagen tendrá más o menos contraste.
...
- Y también podemos usarlos para crear animaciones con un montón de círculos.
...
Me salté la mayoría de las cosas de matemáticas aquí, pero si estás interesado en los principios teóricos de cómo funciona, aquí hay algunas preguntas que puedes usar para guiar tu investigación:
...
¡Esta página web es de código abierto, puede consultar el código en [GitHub](https://github.com/Jezzamonn/fourier)! Si tienes algún comentario o deseas hacer alguna pregunta, no dudes en enviarme un correo electrónico a<span id="email-text"></span> o envíame un tweet en [Twitter](https://twitter.com/jezzamonn).
