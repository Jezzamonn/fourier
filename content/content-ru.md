Преобразование Фурье — инструмент, который используется в самых разных областях. Перед вами объяснение того, как он работает и где может быть полезен. А ещё того, как с помощью него можно делать красивые вещи, например это:

<canvas id="self-draw" class="sketch" width=500 height=500></canvas>

Я объясню, как работает эта анимация и преобразование Фурье.

По завершении вы узнаете:
...
Пока что мы оставим математику и формулы за скобками. За всем этим стоит интересная математика, но лучше начать с того, что конкретно делает это преобразование и почему вам стоит его использовать. Если вы хотите узнать больше о том, как это работает, ниже вы найдете материалы для дальнейшего чтения.
...
Давайте начнем с простых примеров и будем потихоньку продвигаться вперёд. Для начала мы посмотрим на волны - колебания, которые повторяются во времени.
...
С помощью преобразования Фурье мы можем взять исходную волну и получить все входящие в неё синусоиды. В этом примере вы практически можете сделать это в своей голове, просто взглянув на волну. 
...
Самый простой пример это звук - когда мы слушаем его, мы не слышим сложную изогнутую волну, вместо этого мы слышим разные частоты синусоид, которые вместе составляют звук.
...
Ещё мы можем использовать этот метод и на тех волнах, которые не выглядят так, будто они состоят из синусоид.
...
Визуально вы заметите, что только первые две синусоиды делают наибольший вклад. Когда ползунок посередине, волна приобретает нужные очертания, но всё ещё остается немного зубчатой. Нам необходимо добавить оставшиеся маленькие синусоиды, чтобы сгладить углы.
...
Мы можем обратить этот факт себе на пользу. Используя преобразование Фурье, мы можем взять только важные части звука и сохранить их - в результате получив что-то очень похожее на оригинал.
...
Но вместо этого мы можем представить их в виде набора синусоид. После этого мы сможем сжать звуковую дорожку, проигнорировав более маленькие частоты. Конечный результат не будет идентичным, но для человека он будет звучать очень похоже.
...
Теперь давайте погрузимся глубже в преобразование Фурье. Следующая часть не только круто выглядит, но и даёт получше разобраться в том что делает преобразование Фурье. Но в основном круто выглядит.
...
Как и раньше, мы получим довольно неплохое приближение к нашему рисунку всего с несколькими кругами. Поскольку это довольно простой рисунок, всё что делают все остальные круги - это добавляют остроты углам.

И это справедливо для любого рисунка! Теперь ваша очередь поиграться с ними.
...
И снова вы можете заметить, что достаточно небольшого числа кругов чтобы сделать похожий рисунок и не сохранять все его детали.

Можем ли мы использовать это для настоящих данных? Конечно! В реальности, существует ещё один формат данных, называемый SVG, который лучше работает с фигурами которые мы рисуем. Но пока мы просто делаем крутые гифки.
...
Однако одних изображений с горизонтальными и вертикальными линиями по-отдельности недостаточно. Нам понадобятся дополнительные, которые получаются путём перемножения этих двух.
...
Если мы возьмём изображения, откорректируем их контрастность на нужную величину, после чего совместим их, мы сможем создать любую картинку.

Давайте начнем с буквы "А". Она довольно маленькая, но именно это нам и нужно, иначе мы получим слишком много других изображений.
...
Изображение делится на секции размером 8х8, и каждая секция делится сама по себе. Мы используем набор частот чтобы определить насколько светлый или темный каждый пиксель, а затем два других набора частот - для цвета, один для красно-зеленого, а другой для сине-жёлтого. Количество частот, которое мы используем для каждой секции определяет качество JPEG картинки.
...
Давайте подведём итоги:
 ...
 - а ещё мы можем использовать их для того чтобы создавать крутые анимации с кучей кружочков

Всё это лишь поверхностный взгляд с некоторыми сферами применения. Преобразование Фурье - мощнейший инструмент, ведь разделение чего бы то ни было на частоты - это нечто фундаментальное. Оно используется в самых разных областях, включая проектирование схем, сигналах мобильных телефонов, магнитно-резонансных томографах (МРТ), и квантовой физике.
...
Предлагаю вам несколько очень хороших ресурсов, которые помогут узнать ещё больше:
...
Отличная статья которая погружается в математические аспекты происходящего.
...
Меня зовут Джез! Моя основания работа - это [поисковик](https://www.google.com/) из Области залива Сан-Франциско, а в свободное время я делаю игры и интерактивные штуки, такие как эта.
...
Если хотите увидеть больше моих работ, зайдите на [домашнюю страницу](/), а чтобы узнать над чем я собираюсь работать, подпишитесь на меня в Twitter - [@jezzamonn](https://twitter.com/jezzamonn)!
